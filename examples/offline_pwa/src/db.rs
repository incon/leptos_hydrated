...
pub async fn get_db() -> Result<IdbDatabase, String> {
    ...
    let idb_factory = window
        .indexed_db()
        .map_err(|e| format!("{:?}", e))?
        .ok_or_else(|| "no idb".to_string())?;
    let request = idb_factory
        .open_with_u32("offline_pwa_db", 1)
        .map_err(|e| format!("{:?}", e))?;
...
pub async fn set_item(key: &str, value: &str) -> Result<(), String> {
    ...
    let db = get_db().await?;
    let tx = db
        .transaction_with_str_and_mode("state", IdbTransactionMode::Readwrite)
        .map_err(|e| format!("{:?}", e))?;
...
pub async fn get_item(_key: &str) -> Result<Option<String>, String> {
    Ok(None)
}
#[cfg(not(feature = "hydrate"))]
pub async fn set_item(_key: &str, _value: &str) -> Result<(), String> {
    Ok(())
}
